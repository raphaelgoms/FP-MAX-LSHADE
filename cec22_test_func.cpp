...
void cec22_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		sprintf(FileName, "input_data_cec22/M_%d_D%d.txt", func_num, nx);
		...
		/* Load shift_data */
		sprintf(FileName, "input_data_cec22/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		{
			sprintf(FileName, "input_data_cec22/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
