...
Fitness LSHADE::run()
{
  ...
  if (debug_mode) results_root_path = "/home/raphaelgs/code/FIM-CGRASP/results/generations/LSHADE/cec-f" + to_string(function_number);
  ...
  if (debug_mode) {
    int check = mkdir("/home/raphaelgs/code/FIM-CGRASP/results/generations",0777);
    check = mkdir("/home/raphaelgs/code/FIM-CGRASP/results/generations/LSHADE",0777);
  ...
  while (nfes < max_num_evaluations)
  {
    ...
    //cout << "maxtime="<< maxtime << " time="<<(s_CPU_final - s_CPU_inicial) << endl;
    if (maxtime > 0 && (s_CPU_final - s_CPU_inicial) > maxtime) {
      //cout << "TIMEOUT!" << endl;
      break;
    }
  ...
  if (debug_mode) pop_costs_file.close();
