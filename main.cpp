...
int main(int argc, char **argv)
{
	...
	while (++i < argc)
	{
		...
		else if (isEliteTypeCode(argv[i], elite_type))
		{
			string et = argv[i];
			if (print)
				cout << "Elite Type: " << et << endl;
			arguments += et + " ";
		}
		else if (isPatternUsageStrategyCode(argv[i], pattern_use_strategy))
		{
			string puc = argv[i];
			if (print)
				cout << "Pattern usage: " << puc << endl;
			arguments += puc + " ";
		}
		else if (isPatternSelStrategyCode(argv[i], pattern_sel_strategy))
		{
			string psc = argv[i];
			if (print)
				cout << "Pattern seletion: " << psc << endl;
			arguments += psc + " ";
		}
		else if (isFillingStrategyCode(argv[i], filling_strategy))
		{
			string fsc = argv[i];
			if (print)
				cout << "filling_strategy: " << fsc << endl;
			arguments += fsc + " ";
		}
		else if (!strcmp("-i", argv[i]))
		{
			...
			if (print)
				cout << "Instance: " << iFuncNumb << endl;
			// arguments += "-i " + string(argv[i])  + " ";
		}
		else if (!strcmp("--nvar", argv[i]))
		{
			problemDim = atoi(argv[++i]);
			if (print)
				cout << "Number of variables: " << problemDim << endl;
		...
		else if (!strcmp("--gs", argv[i]))
		{
			dmGenStep = atoi(argv[++i]);
			if (print)
				cout << "DM generations step: " << dmGenStep << endl;
			arguments += "--gs " + string(argv[i]) + " ";
		}
		else if (!strcmp("--dmstart", argv[i]))
		{
			dmStartMoment = atoi(argv[++i]);
			if (print)
				cout << "DM start moment: " << dmStartMoment << endl;
			arguments += "--dmstart " + string(argv[i]) + " ";
		}
		else if (!strcmp("--dmfreq", argv[i]))
		{
			...
			if (print)
				cout << "DM frequency: " << argv[i] << endl;
			arguments += "--dmfreq " + string(argv[i]) + " ";
		}
		else if (!strcmp("--ptsz", argv[i]))
		{
			patternPercentUsed = atof(argv[++i]);
			if (print)
				cout << "Percent of pattern used: " << patternPercentUsed << endl;
			arguments += "--ptsz " + string(argv[i]) + " ";
		}
		else if (!strcmp("--sd", argv[i]))
		{

			standardDeviation = atof(argv[++i]);
			if (print)
				cout << "Standard Deviation used: " << standardDeviation << endl;
			arguments += "--sd " + string(argv[i]) + " ";
		}
		else if (!strcmp("--irace", argv[i]))
		{
			irace_mode = true;
			arguments += " --irace ";
		}
		...
		else if (!strcmp("--max_evals", argv[i]))
		{

			max_evals = atoi(argv[++i]);
			if (print)
				cout << "Max num. of evals: " << standardDeviation << endl;
			arguments += "--max_evals " + string(argv[i]) + " ";
		}
		else if (!strcmp("--cec", argv[i]))
		{
			cec_function_number = atoi(argv[++i]);
			if (print)
				cout << "CEC Function: " << cec_function_number << endl;
			// arguments += "--cec " +  string(argv[i])  + " ";
			funcName = function_prefix + "-f" + to_string(cec_function_number) + "-d";
		}
		else if (!strcmp("--rg", argv[i]))
		{
			g_restart_generation = atoi(argv[++i]);
			if (print)
				cout << "Restart Generation Step: " << g_restart_generation << endl;
		...
		else if (!strcmp("--k", argv[i]))
		{
			...
			if (print)
				cout << "Number of patterns to mining: " << number_of_patterns_to_mining << endl;
		...
		else if (!strcmp("--h", argv[i]))
		{
			...
			if (print)
				cout << "Discretization Step: " << discretization_step << endl;
		...
		else if (!strcmp("--er", argv[i]))
		{
			elite_rate = atof(argv[++i]);
			if (print)
				cout << "Elite Rate: " << elite_rate << endl;
			arguments += "--er " + string(argv[i]) + " ";
		}
		else if (!strcmp("--cr", argv[i]))
		{
			clusters_rate = atof(argv[++i]);
			if (print)
				cout << "Clusters Rate: " << clusters_rate << endl;
			arguments += "--cr " + string(argv[i]) + " ";
		}
		else if (!strcmp("--psr", argv[i]))
		{
			patterns_size_rate = atof(argv[++i]);
			if (print)
				cout << "Pattern Size Rate: " << patterns_size_rate << endl;
			arguments += "--psr " + string(argv[i]) + " ";
		}
		else if (!strcmp("--pcr", argv[i]))
		{
			patterns_count_rate = atof(argv[++i]);
			if (print)
				cout << "Patterns Count Rate: " << patterns_count_rate << endl;
		...
		else if (!strcmp("--sf", argv[i]))
		{
			seed_type = "fixed";
			if (print)
				cout << "Seeds type: " << seed_type << endl;
			arguments += "--sf ";
		}
		else
		{
	...
	if (irace_mode)
	{
		...
		{
			searchAlgorithm *_fp_max_lshade = new FP_MAX_LSHADE(cec_function_number);
			// cout << seed << endl;
			((FP_MAX_LSHADE *)_fp_max_lshade)->seed = seed;
			((FP_MAX_LSHADE *)_fp_max_lshade)->number_of_patterns = number_of_patterns_to_mining;
			((FP_MAX_LSHADE *)_fp_max_lshade)->mining_algorithm = mining_algorithm;
			((FP_MAX_LSHADE *)_fp_max_lshade)->pattern_sel_strategy = pattern_sel_strategy;
			((FP_MAX_LSHADE *)_fp_max_lshade)->pattern_usage_strategy = pattern_use_strategy;
			((FP_MAX_LSHADE *)_fp_max_lshade)->filling_strategy = filling_strategy;
			((FP_MAX_LSHADE *)_fp_max_lshade)->elite_type = elite_type;
			((FP_MAX_LSHADE *)_fp_max_lshade)->dm_start_gen = dmStartMoment;
			((FP_MAX_LSHADE *)_fp_max_lshade)->dm_gen_step = dmGenStep;
			...
			// ((FP_MAX_LSHADE*)_fp_max_lshade)->elite_max_size = 50;
			((FP_MAX_LSHADE *)_fp_max_lshade)->elite_max_size = eliteSize;
			((FP_MAX_LSHADE *)_fp_max_lshade)->seed = seed; // Rn[rand_idx];

			float minsup = ((float)support * eliteSize) / 100;
			// cout << "support = " << minsup << endl;

			((FP_MAX_LSHADE *)_fp_max_lshade)->support = minsup;
			((FP_MAX_LSHADE *)_fp_max_lshade)->discretization_step = discretization_step; // Rn[rand_idx];
			((FP_MAX_LSHADE *)_fp_max_lshade)->cec_year = cec_year;

			if (analysis_mode)
			{
				((FP_MAX_LSHADE *)_fp_max_lshade)->debug_mode = true;
				((FP_MAX_LSHADE *)_fp_max_lshade)->config = arguments;
			}

			((FP_MAX_LSHADE *)_fp_max_lshade)->patterns_count_rate = patterns_count_rate;
			((FP_MAX_LSHADE *)_fp_max_lshade)->patterns_size_rate = patterns_size_rate;

			cost = _fp_max_lshade->run();
		}
